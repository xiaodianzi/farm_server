...
public class ConsoleLoginInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        AdminUser user = (AdminUser) request.getSession(true).getAttribute(SysConstant.CURRENT_USER);
        if (user == null) {
            request.getRequestDispatcher("/toLoginByPassword").forward(request, response);
            return false;
        } else {
            return true;
        }
